<?php
// /feesystem/api/feesystem/generate_grant_receipt_pdf.php
session_start();
...
if (empty($grant_id)) {
    echo json_encode(['success' => false, 'message' => 'Grant ID is required']);
    exit;
}
...
// Get grant details
$query = "SELECT g.*, s.school_name, s.school_address, s.school_phone, s.school_email, s.school_logo
          FROM grants g
          LEFT JOIN tblschoolinfo s ON g.school_id = s.id
          WHERE g.id = :grant_id AND g.school_id = :school_id";
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 7, 'GRANT RECEIPT', 0, 1, 'C');
        
        $this->SetFont('Arial', '', 10);
        $this->Cell(0, 5, 'OFFICIAL RECEIPT FOR GRANT FUNDS', 0, 1, 'C');
    ...
    function ReceiptBody()
    {
        ...
        // Two column layout for grant info
        $this->Cell(45, 7, 'Receipt No:', 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(80, 7, $this->grant_info['grant_number'], 0, 0, 'L');
        $this->SetFont('Arial', '', 10);
        $this->Cell(25, 7, 'Date:', 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(40, 7, date('d/m/Y', strtotime($this->grant_info['receipt_date'])), 0, 1, 'L');
        
        $this->SetFont('Arial', '', 10);
        $this->Cell(45, 7, 'Grant Name:', 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(80, 7, $this->grant_info['name'], 0, 0, 'L');
        $this->SetFont('Arial', '', 10);
        $this->Cell(25, 7, 'Source:', 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(40, 7, $this->grant_info['source'], 0, 1, 'L');
        
        $this->SetFont('Arial', '', 10);
        $this->Cell(45, 7, 'Payment Mode:', 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(80, 7, ucfirst($this->grant_info['payment_mode']), 0, 0, 'L');
        if (!empty($this->grant_info['reference_no'])) {
            $this->SetFont('Arial', '', 10);
            $this->Cell(25, 7, 'Reference:', 0, 0, 'L');
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(40, 7, $this->grant_info['reference_no'], 0, 1, 'L');
        } else {
            $this->Ln(7);
        }
        ...
        foreach ($this->distributions as $dist) {
            ...
            $this->Cell(50, 7, $dist['vote_head_alias'], 1, 0, 'L');
            $this->Cell(30, 7, number_format($amount, 2), 1, 1, 'R');
        }
        ...
        // Notes if any
        if (!empty($this->grant_info['notes'])) {
            $this->SetFont('Arial', '', 10);
            $this->Cell(0, 7, 'Notes:', 0, 1, 'L');
            $this->SetFont('Arial', 'I', 9);
            $this->MultiCell(0, 5, $this->grant_info['notes']);
            $this->Ln(5);
        }
...
$pdf->school_info = [
    ...
    'school_address' => $grant['school_address'],
    'school_phone' => $grant['school_phone'],
    'school_email' => $grant['school_email'],
    'school_logo' => $grant['school_logo'] ?? null
];
$pdf->grant_info = $grant;
$pdf->distributions = $distributions;
$pdf->AddPage();
$pdf->ReceiptBody();

// Output PDF
$pdf_filename = 'grant_receipt_' . $grant['grant_number'] . '_' . date('Ymd') . '.pdf';

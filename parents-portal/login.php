...
$page_title = "Parents Portal Login - EduScore";
...
<head>
    ...
    <meta name="description" content="Parents Portal Login - Access your child's academic progress, exam results, fee balances, and school announcements.">
...
<section class="login-section">
    <div class="container">
        <div class="form-card reveal">
            ...
            <p class="form-subtitle">Sign in to access your child's information</p>
            ...
            <form id="loginForm">
                ...
                <div class="register-link">
                    <p>First time? <a href="register.php">Activate Account</a></p>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    const savedTheme = localStorage.getItem('parentLoginTheme');
    if (savedTheme === 'dark') {
    ...
    themeToggle.addEventListener('click', () => {
        ...
        localStorage.setItem('parentLoginTheme', isDark ? 'dark' : 'light');
        themeText.textContent = isDark ? 'Light Mode' : 'Dark Mode';
    });
    
    // Phone number formatting - ensure only digits and limit to 9 digits
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!phoneRegex.test(phone)) {
            showError('Please enter a valid phone number (9 digits after 254)');
            return;
        }
        
        // Add +254 prefix for API
        ...
        try {
            const response = await fetch('api/parents_login.php', {
                method: 'POST',
            ...
            if (data.success) {
                // Store parent info in session storage for dashboard
                if (data.data) {
                    sessionStorage.setItem('parent_data', JSON.stringify(data.data));
                }
                window.location.href = data.redirect || 'dashboard.php';
            } else {
                showError(data.message || 'Invalid credentials. Please check your phone number and password.');
